class NextChat {
  ...
  static #imgModels = ["animagen", "prodia", "mageai"];
  ...
  static async #fetchData(prompt, model, imgUrl = "") {
    if (NextChat.#chatModels.includes(model)) {
      ...
      if (model === "animagen") {
        return await fetch(`${api}api/animagen?prompt=${prompt}`);
      }
      if (model === "prodia") {
        return await fetch(`${api}v1/generate?prompt=${prompt}`);
      }
      if (model === "mageai") {
        return await fetch(`${api}api/mageai?prompt=${prompt}`);
      }
    } else if (model === "upscale") {
      return await fetch(`${api}upscale?url=${prompt}`);
    } else if (model === "tts") {
      return await fetch(`${api}tts?text=${prompt}`);
    } else {
